...
public class FIService {
    ...
    public FIXML getCustomAcctDetails(ExecuteService request) throws DatatypeConfigurationException {
        ...
        response.getFIXML().getHeader().getResponseHeader().getResponseMessageInfo().setMessageDateTime(request.getArg00().getFIXML().getHeader().getRequestHeader().getRequestMessageInfo().getMessageDateTime());
